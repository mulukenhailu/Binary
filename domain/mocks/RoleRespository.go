// Code generated by mockery v2.46.3. DO NOT EDIT.

package mocks

import (
	context "context"

	domain "github.com/mulukenhailu/Binary/domain"
	mock "github.com/stretchr/testify/mock"
)

// RoleRespository is an autogenerated mock type for the RoleRespository type
type RoleRespository struct {
	mock.Mock
}

// Create provides a mock function with given fields: c, role
func (_m *RoleRespository) Create(c context.Context, role *domain.RoleDto) error {
	ret := _m.Called(c, role)

	if len(ret) == 0 {
		panic("no return value specified for Create")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, *domain.RoleDto) error); ok {
		r0 = rf(c, role)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// Delete provides a mock function with given fields: c, roleId
func (_m *RoleRespository) Delete(c context.Context, roleId int32) error {
	ret := _m.Called(c, roleId)

	if len(ret) == 0 {
		panic("no return value specified for Delete")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, int32) error); ok {
		r0 = rf(c, roleId)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// FetchByName provides a mock function with given fields: c, roleName
func (_m *RoleRespository) FetchByName(c context.Context, roleName string) (domain.RoleDto, error) {
	ret := _m.Called(c, roleName)

	if len(ret) == 0 {
		panic("no return value specified for FetchByName")
	}

	var r0 domain.RoleDto
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, string) (domain.RoleDto, error)); ok {
		return rf(c, roleName)
	}
	if rf, ok := ret.Get(0).(func(context.Context, string) domain.RoleDto); ok {
		r0 = rf(c, roleName)
	} else {
		r0 = ret.Get(0).(domain.RoleDto)
	}

	if rf, ok := ret.Get(1).(func(context.Context, string) error); ok {
		r1 = rf(c, roleName)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// FetchRoles provides a mock function with given fields: c
func (_m *RoleRespository) FetchRoles(c context.Context) ([]domain.RoleDto, error) {
	ret := _m.Called(c)

	if len(ret) == 0 {
		panic("no return value specified for FetchRoles")
	}

	var r0 []domain.RoleDto
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context) ([]domain.RoleDto, error)); ok {
		return rf(c)
	}
	if rf, ok := ret.Get(0).(func(context.Context) []domain.RoleDto); ok {
		r0 = rf(c)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]domain.RoleDto)
		}
	}

	if rf, ok := ret.Get(1).(func(context.Context) error); ok {
		r1 = rf(c)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// Update provides a mock function with given fields: c, role
func (_m *RoleRespository) Update(c context.Context, role *domain.RoleDto) error {
	ret := _m.Called(c, role)

	if len(ret) == 0 {
		panic("no return value specified for Update")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, *domain.RoleDto) error); ok {
		r0 = rf(c, role)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// NewRoleRespository creates a new instance of RoleRespository. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewRoleRespository(t interface {
	mock.TestingT
	Cleanup(func())
}) *RoleRespository {
	mock := &RoleRespository{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
